"""The tests for BlueCurrent device triggers."""
import pytest
...
async def test_if_vehicle_status_fires_on_state_change(hass: HomeAssistant, calls):
    ...
    await hass.async_block_till_done()
    assert len(calls) == 4
    assert calls[3].data["some"] == "with_ventilation"

    hass.states.async_set("sensor.vehicle_status_101", "no_power")
    await hass.async_block_till_done()
    assert len(calls) == 5
    assert calls[4].data["some"] == "no_power"

    hass.states.async_set("sensor.vehicle_status_101", "vehicle_error")
    await hass.async_block_till_done()
    assert len(calls) == 6
    assert calls[5].data["some"] == "vehicle_error"
